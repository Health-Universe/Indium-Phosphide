...
if st.session_state.current != None:
 ...
 # Creating solvent questions
        ...
        with row2_3:
            ...
            sol_3 = st.radio("What is the third solvent?",
                            ('None',
                            'octadecene - ODE',
                            'oleylamine',
                            'dioctylamine',
        ...
        row2a_1, row2a_2, row2a_3 = st.columns(3)
        ...
        with row2a_3:
            ...
            sol_3_amount = st.number_input(label='How much of the third solvent is used? (mL)', value=0.00)
            amount_test(sol_3, sol_3_amount)
        ...
        df_InP_1 = Path(__file__).parents[0] / 'flo_dataset.csv'
        ...
        # Loading ML model for 3 outputs
        InP_model = joblib.load(Path(__file__).parents[0] / 'model_MO_ExtraTrees_c.joblib')
        

        # Make predictions

        In_predicted = InP_model.predict(X_InP)
        ...
        if predict:
            ...
            with c2:
                ...
                st.write('Predicted diameter is', round(In_predicted[0, 0], 3))
                st.write('Predicted absorbance max is', round(In_predicted[0, 1], 3))
                st.write('Predicted emission is', round(In_predicted[0, 2], 3))
