...
if st.session_state.current != None:
 ...
 # Creating solvent questions
        ...
        diameter_model = joblib.load(Path(__file__).parents[0] / 'model_SO_diameter_DecisionTree_c.joblib')
        abs_model = joblib.load(Path(__file__).parents[0] / 'model_SO_abs_ExtraTrees_c.joblib')
        emission_model = joblib.load(Path(__file__).parents[0] / 'model_SO_emission_ExtraTrees_c.joblib')
